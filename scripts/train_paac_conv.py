...
def main(args):
    ...
    shutil.rmtree(args.debugging_folder, ignore_errors=True)
    logging.debug('Configuration: {}'.format(args))

    network_creator, env_creator = get_network_and_environment_creator(args)

    learner = PAACLearner(network_creator, env_creator, args, SwarmRunner, state_processor=SwarmStateProcessor())
...
def get_network_and_environment_creator(args, random_seed=3):
    env_creator = environment_creator.SwarmEnvironmentCreator()
